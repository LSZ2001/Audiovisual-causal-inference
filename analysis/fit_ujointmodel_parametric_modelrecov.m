function [out_struct] = fit_ujointmodel_parametric_modelrecov(iter,prior_type,hetero_type,lapse_type, rescale_aud, prior_type_modelrecovdata, hetero_type_modelrecovdata, lapse_type_modelrecovdata, rescale_aud_modelrecovdata, num_inits, modelrecovdata_path, model_path_temp)
    ...
    datafilename_basis = 'fittedparams_UJoint_'+hetero_type_modelrecovdata+"-"+prior_type_modelrecovdata;
    if(rescale_aud_modelrecovdata=="free")
        datafilename = datafilename_basis + "_rescalefree";
    elseif(rescale_aud_modelrecovdata=="1")
        datafilename = datafilename_basis + "_rescale1";
    elseif(rescale_aud_modelrecovdata=="4/3")
        datafilename = datafilename_basis + "_rescale4over3";
    end
    datafilename = datafilename + "_lapse"+lapse_type_modelrecovdata + "_modelrecovdata";

    % Final file contains both dataset info and model info
    filename_final = filename + "__" + extractBetween(datafilename, "fittedparams_UJoint_", "_modelrecovdata", "Boundaries", "exclusive");

    filename
    datafilename
    filename_final

    % Test if resultant file name would be too long
    %save(model_path_temp+filename_final+"_"+num2str(iter)+"150", 'filename_final');
    ...
    % Load fake data for model recovery.
    load(modelrecovdata_path+ datafilename);
    data_UV = fakedata_UV; % last argument is is_visual.
    data_UA = fakedata_UA;
    clear fakedataUV  fakedataUA;
    ...
    close all;
    save(model_path_temp+filename_final+"_"+num2str(iter), 'out_struct','model_spec');
end
