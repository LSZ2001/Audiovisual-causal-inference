...
%     bincenters = binedges(1:(end-1)) + (binedges(2:end) - binedges(1:(end-1))) ./2;    
    ...
    PMR_vals = zeros(3, num_subjects, 2, num_binedges-1); % Rels -> subjects -> C=1 or C=2 -> bins.
    Mean_Biases = zeros(3, num_subjects, num_binedges-1);
...
% --------------For model fit visualization -- omitted for now!-----------
            ...
            bincenters_stratrels(i,:) = bincenters;
            %Prob_C1(isnan(Prob_C1))=0;
            Mean_Biases(l,i,:) = mean_biases;
            Mean_Biases_modelfit(l,i,:) = mean_biases_modelfit;
    
        end
    % Plot average plots
    bincenters_stratrels_avg = mean(bincenters_stratrels, 1);
    bincenters_stratrels_std = std(bincenters_stratrels, [], 1);
    
    rel_colors = brewermap(9,"Set1");
    rel_colors = rel_colors([1,2,3],:);
    Mean_Biases_avg = squeeze(nanmean(Mean_Biases, 2));
    Mean_Biases_sem = squeeze(nanstd(Mean_Biases, [], 2)) ./ sqrt(squeeze(sum(~isnan(Mean_Biases), 2)));   
    Mean_Biases_modelfit_avg = squeeze(nanmean(Mean_Biases_modelfit, 2));
    Mean_Biases_modelfit_sem = squeeze(nanstd(Mean_Biases_modelfit, [], 2)) ./ sqrt(squeeze(sum(~isnan(Mean_Biases_modelfit), 2)));   
    ...
    curve2 = Mean_Biases_modelfit_avg(l,:) - Mean_Biases_modelfit_sem(l,:);
    curve1(isnan(curve1))=0; curve2(isnan(curve2))=0;
    x2 = [bincenters_stratrels_avg, fliplr(bincenters_stratrels_avg)];
