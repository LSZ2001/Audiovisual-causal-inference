...
function [output] = NLLfun_BAV_parametric(ModelComponents_V, ModelComponents_A, theta_raw, R, S_V, S_A, PMIntegrationParams, return_predictive_samples, return_response_distr, plot_consider_lapse, dx_max, causal_inf_strategy, lapse_type, Gaussian_lapse_SD)
    ...
    elseif(nargin<9) % if return posterior predictive samples and not vectorized response distribution, default is not include lapse mixture component.
        return_response_distr=false; plot_consider_lapse=true; dx_max=0.5; causal_inf_strategy="ProbMatching";
        lapse_type = "Uniform";  Gaussian_lapse_SD = NaN;
    elseif (nargin<10) % if return vectorized response distribution (need S to be one s-value and rel-level repmat, and R to be r_grid), default is not include lapse mixture component.
        plot_consider_lapse=true; dx_max=0.5; causal_inf_strategy="ProbMatching"; 
        lapse_type = "Uniform";  Gaussian_lapse_SD = NaN;
    elseif(nargin<11)
        lapse_type = "Uniform";  Gaussian_lapse_SD = NaN;
    elseif(nargin<12)
    ...
    if(motor_noise_type=="Gaussian")
        ...
        if(return_predictive_samples) % If return predictive samples, cannot return response distribution despite input args.
            ...
            sigma_vis = sigma_fun_vis(S_V, sigma0_vis, k_vis) .* rel_multiply_factor;
            sigma_aud = sigma_fun_aud(S_A, sigma0_aud, k_aud);
            ...
            p_xV_given_s_integrate = 1./(sqrt(2.*pi).*sigma_fun_vis(s_grid_integrate,sigma0_vis,k_vis).*rel_multiply_factor) .* exp(-((x_V_rescale-s_grid_integrate).^2)./(2.*((sigma_fun_vis(s_grid_integrate,sigma0_vis,k_vis).*rel_multiply_factor).^2)));
            p_xA_given_s_integrate = 1./(sqrt(2.*pi).*sigma_fun_aud(s_grid_integrate,sigma0_aud,k_aud)) .* exp(-((x_A_rescale-s_grid_integrate).^2)./(2.*((sigma_fun_aud(s_grid_integrate,sigma0_aud,k_aud)).^2)));
            ...
            switch lapse_type
                case "Uniform"
                    % Random responses (uniform in -45:1:45)
                    lapse_val = -45 + (45+45)*rand(sum(lapse_idx),1);
                case "Gaussian"
