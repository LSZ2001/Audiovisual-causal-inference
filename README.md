...
- **fit_models_modelrecov_gpc.m** performs model recovery (and parameter recovery) for specific unisensory models. It takes in simulated datasets generated by **ujointmodel_parametric_modelrecov_datagen.m** and fits models over them. The number of iters per subject in the code is used to generate the saved *.mat* files (model fits) in the *modelfits* folder.
- **manuscript_allplots.m** creates all figures for the manuscript, based on the datafiles and the saved model fits.
- **create_composite_allfits_figures.ipynb** combines some subpanel figures, renames them based on order of appearance, and converts them to .eps format.
...
- The *model_recov* subsubfolder contains simulated unisensory datasets generated by **ujointmodel_parametric_modelrecov_datagen.m**.
...
parametric on UA+UV simulated data (model recovery):
Parallelized for HPC. It uses the same optimization code and model-naming convention as above;
'fittedparams_UJoint'+ fitted_model_name + '__' + sim_dataset_name/
...
### Model recovery code
**ujointmodel_parametric_modelrecov_datagen.m** generates simulated datasets from human-fitted parameters of models. These simulated datasets are used for model and parameter recovery. 
