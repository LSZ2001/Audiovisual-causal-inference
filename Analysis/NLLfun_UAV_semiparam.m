...
function [output] = NLLfun_UAV_semiparam(theta, data, return_predictive_samples, return_response_distr, plot_consider_lapse, lapse_type, Gaussian_lapse_SD)
    ...
    lapse_range = [-45,45];
    has_rescale_vis = false; % fit k_vis
    has_rescale_aud = true; % fit k_aud
    ...
    % Get s_grid_integrate to integrate over.
    binedges = linspace(a,b,num_bins+1);
    binwidth =  binedges(2)-binedges(1);
    s_grid_integrate = (binedges(1:end-1) + binwidth/2)';
        
    % Concatenate data_UA and data_UV.
    num_subjects = length(data);
   
    % Dataset parsing
    LL_idx_mat = cell(1,num_subjects);
    subjidx=1; % Always 1, since we need to fit 1 subject at a time. 
    s_matrix = data{subjidx};
    S = s_matrix(:,1); R_raw = s_matrix(:,2); reliabilities = s_matrix(:,3);
    LL_idx_mat{subjidx} = zeros(length(S),1);
    ...
    sigma_fun_all_pivots = [sigma_fun_vis_rel_high_pivots;sigma_fun_vis_rel_high_pivots;sigma_fun_vis_rel_high_pivots;sigma_fun_aud_pivots];
    
    prior_pivots = cumsum([1,theta((2*num_pivots+1):(3*num_pivots-1))]);
    prior_pivots = [fliplr(prior_pivots(2:end)), prior_pivots];
    
    rel_multiply_factors =[1, theta((3*num_pivots+0):(3*num_pivots+1)),1];
    lapse = theta(3*num_pivots+2);  sigma_motor = theta(3*num_pivots+3);
    rescale_vis = 1; rescale_aud = theta(end);
 ...
 %% Below is visualization code (posterior pred samples) 
    if(return_predictive_samples) % If return predictive samples, cannot return response distribution despite input args.
        ...
        output = cell(1,num_subjects);
        for subjidx=1
            % Assume input only contains one reliability level (1 to 4)!!
            rel_level = reliabilities(1);
            rescale = rescales(rel_level);
            rel_multiply_factor = rel_multiply_factors(rel_level);
            sigma_fun_pivots = sigma_fun_all_pivots(rel_level,:);      
            
            % Get normalized prior density.
            p_s = exp(interp1(s_pivot_full, (prior_pivots), s_grid_integrate, 'pchip'));
            p_s = p_s / qtrapz(p_s*(s_grid_integrate(2)-s_grid_integrate(1)));
            log_prior = log(p_s);
            
            sigma_fun = @(s) min([repmat(45,length(s),1) , interp1(s_pivot_full, exp(sigma_fun_pivots), s, 'pchip')], [], 2);       
            ...
            x_min = min(-45,s_min(1) - 5*sigma_fun(s_min(1)).*rel_multiply_factor);
            x_max = max(45,s_max(1) + 5*sigma_fun(s_max(1)).*rel_multiply_factor);

            x_grid = x_min:0.1:x_max;
            %x_grid = -45:0.5:45;

            Nx = length(x_grid); % originally 2^10
            %[x_min,x_max]
            dx = x_grid(2) - x_grid(1);

            log_p_x_given_s = -log(sigma_fun(S).*rel_multiply_factor) - 0.5.*log(2*pi) - 0.5.*((x_grid-S)./(sigma_fun(S).*rel_multiply_factor)).^2;
            ...
            %[s_hats_PM,~] = midpoint_trepazoid_getpostmean_nonparam(g_fun, p_s_fun, x_obs_vals, rescale, sigma0s(reliabilities), a, b, num_bins, "midpoint");
            % Add motor noise
            samps = s_hats_PM + sigma_motor .* randn([length(S),num_samps]);
            % Add in lapse trials
            ...
            samps(find(lapse_idx~=0)) = lapse_val;

            output{subjidx} = samps;
        end
        return
    end
        
%% Below is all Bayesian integration code for computing NLL.           
    for i=1:num_rels 
        ...
        num_trials = length(rel_trial_idx);

        % Get reliability and sigma(s) pivots for this modality.
        rescale = rescales(i);
        rel_multiply_factor = rel_multiply_factors(i);
        sigma_fun_pivots = sigma_fun_all_pivots(i,:);
        sigma_fun = @(s)  min([repmat(45,length(s),1) , interp1(s_pivot_full, exp(sigma_fun_pivots), s, 'pchip')], [], 2);       
...
%         ll_idx_param = lapse./(max(lapse_range)-min(lapse_range)) + (1-lapse).* prob_r_given_s_nolapse_param; % Account for lapse (Unif(lapse_range) part of the mixture)
        ...
        LL_idx_mat{subjidx}(rel_trial_idx) = ll_idx;
...
LL_idx = LL_idx_mat{1}; % Just 1 subject, hence always {1}
